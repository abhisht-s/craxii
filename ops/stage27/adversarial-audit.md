...
| Effective/permitted/inheritable/ambient capabilities | exactly `CAP_KILL`; bounded transition set is KILL/SETGID/SETUID/SETPCAP | exactly KILL inside delegated root | same | normally none | depends on profile | parent service boundary; child all zero | B/C now reproduce the production parent capability sets instead of relying on an unrelated cross-delegation capability. Child zero sets are independently required. |
| Capability bounding set | minimal transition set | exact production transition set | same | host login set | previously unexamined | launcher now drops every child bound capability | Retaining the bounding set was a **SECURITY_BOUNDARY** defect. `no_new_privs` reduced exploitability but did not satisfy “all capabilities cleared.” |
...
| Service cgroup membership | `/system.slice/craxii-server.service` | test parent first enters the delegated execution root | same | none | ordinary harness none; Linux profile configured | parent service plus per-execution UUID child | B/C begin within the production delegation before dropping to the server identity. For the restart case only, the outside root controller moves the test worker back to its own cgroup after the owned execution has started so the observer survives the unit restart. |
| Delegated authority and parent cgroup | `Delegate=yes`; backend owns its subtree | parent already inside delegated root | same | none | selected profile only | backend creates UUID child below configured root | Live workers use normal delegated ancestry and no longer attempt cross-boundary migration. The real CAP_KILL-only service separately proves create/place/execute/remove during startup. |
...
| Signal authority | service has `CAP_KILL`; systemd owns control group | KILL parent; child none | same | caller | mocks or selected Linux path | backend can signal other-UID child; child cannot regain authority | Signal, timeout, cancellation, and service-restart results require distinct canonical evidence without extra verifier-worker authority. |
...
| systemd lifecycle ownership | Restart=on-failure, KillMode=control-group, TimeoutStopSec=30s | terminal/cancel workers remain inside; the restart observer is moved out by the root controller only after spawn | same delegated runner | Cargo owns process | harness owns server tasks | production systemd owns backend | Synthetic restart validates cgroup cleanup while keeping LocalWorkstation construction/spawn production-like. It still does not replace active-work proof hosted by the real backend. |
...
| S27-AUD-013 | P1 HOST_PROOF_GAP | real service vs B/C | A CAP_SYS_ADMIN verifier child was assumed to prove the CAP_KILL-only service can create/place/clean its own execution. | The alternate cross-delegation path instead failed cgroup-v2 common-ancestor write authorization and collapsed to `unsupported_capability`. | Production startup retains its combined probe; every live/unit test worker now enters the delegated root before dropping identity, uses the production CAP_KILL/transition bounding sets, and reports a safe launcher-probe failure code. | **Required.** |
...
CAP_KILL-only delegated verifier workers; a prior provider benchmark is not to be rerun.
