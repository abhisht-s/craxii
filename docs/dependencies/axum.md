...
- Feature policy: Features are limited to the implemented HTTP/JSON/template tracing/query/WebSocket behavior.
...
- Transitive notes: Brings reviewed `axum-core`, Hyper/HTTP implementation crates, `matchit
  ...
  The repository therefore explicitly admits the OSI-approved BSD-3-Clause license in `deny.toml`.
...
- Review: Approved by the repository/project owner on 2026-08-28 for the public HTTP/WebSocket adapter.
