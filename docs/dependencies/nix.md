...
- Dependency kind and owner: Direct normal dependency confined to Unix SQLite lifecycle support,
  LocalWorkstation process cleanup, and the Stage 27 terminal-controlled benchmark runner.
...
- Declaration: `version = "0.31"`, default features disabled, exactly `fs`, `process`, `signal`,
  and `term` enabled. `process` is the required feature support for the approved signal APIs.
...
- Feature rationale: `fs` exposes statfs/flock; `signal` exposes typed SIGTERM/SIGKILL operations;
  `process` exposes the typed `Pid` support required by those operations; `term` exposes termios
  control used only to disable echo while the Stage 27 benchmark reads its bearer from the TTY.
  Socket, user, mount, and other convenience features remain disabled.
- Graph and build script: resolves `bitflags`, `cfg-if`, `libc`, and build dependency
  ...
  compiler.
- Native/unsafe implications: nix wraps platform `statfs(2)`, `flock(2)`, `kill(2)`, termios, and
...
- Review date and approval: 2026-09-09, feature expansion explicitly approved by the
  repository/project owner for the current Unix workstation adapter and Stage 27 benchmark.
