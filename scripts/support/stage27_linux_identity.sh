...
install -o root -g root -m 0555 /craxii-target/debug/craxii-server /opt/craxii/releases/test/craxii-server
install -o root -g root -m 0555 /craxii-target/debug/craxii-admin /opt/craxii/releases/test/craxii-admin
install -o root -g root -m 0555 /craxii-target/debug/craxii-workstation-reader /opt/craxii/releases/test/craxii-workstation-reader
...
runuser -u craxii-server -- /usr/bin/env CRAXII_BACKEND_CANARY=synthetic-environment-canary \
  ...
  "test \"\$(id -un)\" = craxii; \
   ...
   ! /bin/bash --noprofile --norc -c \"printf x >>'${trusted_binary}'\" 2>/dev/null; \
