...
#[ignore = "run by scripts/verify-stage21-macos-client with the native probe"]
async fn real_native_client_transport_converges_and_cancels_against_localhost_backend() {
    let probe = std::env::var_os("CRAXII_STAGE21_PROBE").map(PathBuf::from);
    let Some(probe) = probe.filter(|path| path.is_file()) else {
        panic!("CRAXII_STAGE21_PROBE must identify the prebuilt native integration executable");
    };

    let commit_gate = ScriptGate::new();
    let cancellation_gate = ScriptGate::new();
    let mut provider_programs = programs(&[
        ProgramPlan::Answer {
            text: "stage21 authoritative answer".to_owned(),
    ...
    let mut child = tokio::process::Command::new(probe)
        .env("CRAXII_STAGE21_INTEGRATION", "1")
        .env(
            "CRAXII_STAGE21_ENDPOINT",
            format!("http://{}/", harness.authority),
        )
        .env("CRAXII_STAGE21_TOKEN", &harness.bearer)
        .env("CRAXII_STAGE21_PROFILE_ID", &profile_id)
        .env("CRAXII_STAGE21_STATE_DIR", state_directory)
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .kill_on_drop(true)
        .spawn()
        .expect("spawn native Stage 21 integration client");
    let stdout = child.stdout.take().expect("capture native probe stdout");
    let mut lines = BufReader::new(stdout).lines();
    let mut observations = Vec::new();
    let read_result = tokio::time::timeout(Duration::from_secs(60), async {
        while let Some(line) = lines.next_line().await.expect("read native probe output") {
            if line == "DRAFT_OBSERVED" {
                commit_gate.release();
            }
            let complete = line == "STAGE21_NATIVE_INTEGRATION_PASSED";
    ...
    for expected in [
        ...
        "BOOTSTRAP_CONVERGED",
        "STAGE21_NATIVE_INTEGRATION_PASSED",
    ] {
        assert!(
    ...
    assert_eq!(
        captures.len(),
        2,
        "both native commands must reach ScriptedProvider"
    );
}
